(() => {
  ...
  async function init() {
    ...
    const forceVersion = () => {
      ...
      if (version && version.textContent !== "v0.7.7") version.textContent = "v0.7.7";
    };
    forceVersion();
  ...
  init().catch(error => console.error("Falha ao carregar dashboard v0.7.7", error));
})();
...
;(() => {
  ...
  import("./history-charts-v075.js?v=0.7.5")
    .catch(error => console.error("Falha ao carregar gráficos do Histórico v0.7.7", error));
})();
